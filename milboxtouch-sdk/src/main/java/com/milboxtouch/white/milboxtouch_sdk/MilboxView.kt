...
fun <T> Observable<T>.list(): Observable<List<T>> {
    return this.scan(listOf<T>(), { list, current ->
...
class MilboxView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : View(context, attrs, defStyle) {
    ...
    //events
    fun addOnSetupProgressListener(listener: () -> Void) {
        onSetupProgressListeners.add(listener)
    }

    fun removeOnSetupProgressListener(listener: () -> Void) {
        onSetupProgressListeners.remove(listener)
    }

    fun addOnSetupCompletedListener(listener: () -> Void) {
        onSetupCompletedListeners.add(listener)
    }

    fun removeOnSetupCompletedListener(listener: () -> Void) {
        onSetupCompletedListeners.remove(listener)
    }
    ...
    fun removeOnTapListener(listener: () -> Void) {
    ...
    fun removeOnDoubleTapListener(listener: () -> Void) {
        onDoubleTapListeners.remove(listener)
    }

    fun addOnScrollListener(listener: (degree: Float) -> Void) {
        onScrollListeners.add(listener)
    }

    fun removeOnScrollListener(listener: (degree: Float) -> Void) {
        onScrollListeners.remove(listener)
    }

    fun addOnScrollBeganListener(listener: () -> Void) {
        onScrollBeganListeners.add(listener)
    }

    fun removeOnScrollBeganListener(listener: () -> Void) {
        onScrollBeganListeners.remove(listener)
    }

    fun addOnScrollEndedListener(listener: () -> Void) {
        onScrollEndedListeners.add(listener)
    }

    fun removeOnScrollEndedListener(listener: () -> Void) {
        onScrollEndedListeners.remove(listener)
    }

    fun addOnSwipeListener(listener: (Swipe) -> Void) {
        onSwipeListeners.add(listener)
    }

    fun removeOnSwipeListener(listener: (Swipe) -> Void) {
...
//        Observable.just("STR1","STR2","STR3").pre().subscribe{
//            Log.e("aaa","TEST pre:${it.first}  current:${it.second}")
//        }
    }
...
//        }
        ...
        }.zipWith(scrollStroke.count(), { a, b -> Pair(a, b) }).repeat().subscribe().autoUnSubscribe()
        ...
        scrollStroke.repeat().subscribe {
            onScrollSubject.onNext(it)//TODO:scrollが負にならないバグ
            Log.e("aaa", "scroll")
        }.autoUnSubscribe()
//
